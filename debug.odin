...
print_cpu :: proc(cpu: ^CPU) {
    ...
    log.debugf("AF: %x, BC: %x, DE: %x, HL: %x, SP: %x, PC: %x | Z: %d, H: %d, N: %d, C: %d | IME: %d", cpu.af, cpu.bc, cpu.de, cpu.hl, cpu.sp, cpu.pc, z, h, n, c, ime)
}
...
draw_debugger_info :: proc(gb: ^GB) {
    ...
    cpu_str := rl.TextFormat("AF: %4x, BC: %4x, DE: %4x, HL: %4x, SP: %4x, PC: %4x | %s | IME: %s", gb.cpu.af, gb.cpu.bc, gb.cpu.de, gb.cpu.hl, gb.cpu.sp, gb.cpu.pc, flags_str, ime_str)
