...
ppu_init :: proc(ppu: ^PPU, mem: ^GB_Memory) {
    ...
    ppu.states[PPU_Mode.VBlank] = create_state(vlank_on_enter, nil, vblank_duration, PPU_Mode.OAM_Scan)
    ppu.states[PPU_Mode.OAM_Scan] = create_state(oam_scan_on_enter, nil, OAM_Scan_Dots, PPU_Mode.DrawPixels)
    ppu.states[PPU_Mode.DrawPixels] = create_state(nil, draw_pixels_on_update, 172, PPU_Mode.HBlank)
...
scan_oam :: proc(ppu: ^PPU, mem: ^GB_Memory) {
    ...
    for i: u16 = 0; i < OAM_EntryCount && sa.len(ppu.scanline_objects) < MaxObjectsPerScanline; i += 1 {
        obj_addr := oam_addr + (i * 4)
        y_pos := mem_read(mem, obj_addr)
...
create_state :: proc(on_enter: proc(gb: ^GB), on_update: proc(gb: ^GB, tick: u64), duration_ticks: int, next_mode: PPU_Mode) -> PPU_State {
